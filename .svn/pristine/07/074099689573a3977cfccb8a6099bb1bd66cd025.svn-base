...
public class CaseApplicationNCPController extends BasicAnnotatedFormController
{
	...
	public String processNCPParticipant(HttpServletRequest request, Model model)
	{
		...
		try
		{
			...
			if (action.equals(AppConstants.NCP_DELETE))
			{
				...
				{
					// model.addAttribute("message", "Problem encountered during
					// delete!!");
					throw new Exception("CaseApplicationNCPController: processNcpParticipant: NCP delete failed. Return code: " + rValue);
				}

				List<CaseParticipant> ncpPartList = casePartService.getParticipantByPartType(applicationId, AppConstants.NCP_PARTICIPANT_TYPE);

				if (ncpPartList.size() == 0)
				{
					// update step progress
					rValue = workFlowService.updateStepNumber(applicationId, AppConstants.FLOW_TYPE_APPL_SCREEN, 4, ncId);

					if (rValue != null && !rValue.substring(0, 4).trim().equals(AppConstants.OPERATION_SUCCESS))
					{
						// model.addAttribute("message", "Problem encountered
						// during saving!!");
	...
	public String getNCPDetails(HttpServletRequest request, Model model)
	{
		...
		try
		{
			...
			if (session.getAttribute("selectedNCP") != null)
			{
				...
				List<CasePartNCPExt> ncpExtDetailsList = casePartService.getCasePartNCPExtByPartId(applicationId, ncpPartId);
				...
				{
					List<CaseCourtOrder> ncpOrderList = casePartService.getCaseCourtOrderByPartId(applicationId, ncpPartId);
					...
					if (ncpOrderList.size() > 0)
					{
						...
						for (int i = 0; i < ncpOrderList.size(); i++)
						{
							...
							if (orderNum != null && !orderNum.equals(""))
							{
								...
								List<CasePartOrder> selectChildList = casePartService.getCasePartOrderByOrderNum(orderNum);
			...
			model.addAttribute(AppConstants.CODE_LOOKUP_MILITARY_LIST, (List<CodeLookUp>) ctx.getAttribute(AppConstants.CODE_LOOKUP_MILITARY_LIST));
			...
			model.addAttribute(AppConstants.CODE_LOOKUP_MALBRANCH_LIST, (List<CodeLookUp>) ctx.getAttribute(AppConstants.CODE_LOOKUP_MALBRANCH_LIST));
			model.addAttribute(AppConstants.CODE_LOOKUP_HAIRCOLOR_LIST, (List<CodeLookUp>) ctx.getAttribute(AppConstants.CODE_LOOKUP_HAIRCOLOR_LIST));
			model.addAttribute(AppConstants.CODE_LOOKUP_EYECOLOR_LIST, (List<CodeLookUp>) ctx.getAttribute(AppConstants.CODE_LOOKUP_EYECOLOR_LIST));
			model.addAttribute(AppConstants.CODE_LOOKUP_PAYFREQ_LIST, (List<CodeLookUp>) ctx.getAttribute(AppConstants.CODE_LOOKUP_PAYFREQ_LIST));
			...
			@ModelAttribute("partInfo") CaseParticipant partInfo) throws Exception
	{
		...
		try
		{
			...
			// mailing address
			helper.populateAddress(pageData, paramList, AppConstants.ADDRESS_MAILING, AppConstants.ADDRESS_MAILING_CODE, AppConstants.NCP_PARTICIPANT_TYPE);

			if (paramList.get("res_addr") == null)
			{
				helper.populateAddress(pageData, paramList, AppConstants.ADDRESS_RESIDENTIAL, AppConstants.ADDRESS_RESIDENTIAL_CODE, AppConstants.NCP_PARTICIPANT_TYPE);
			}

			helper.populateAddress(pageData, paramList, AppConstants.ADDRESS_OTHER, AppConstants.ADDRESS_OTHER_CODE, AppConstants.NCP_PARTICIPANT_TYPE);

			// contact information
			helper.populateContact(pageData, paramList, AppConstants.CONTACT_TYPE_HOME_CODE, AppConstants.CONTACT_TYPE_HOME, AppConstants.NCP_PARTICIPANT_TYPE);
			helper.populateContact(pageData, paramList, AppConstants.CONTACT_TYPE_WORK_CODE, AppConstants.CONTACT_TYPE_WORK, AppConstants.NCP_PARTICIPANT_TYPE);
			helper.populateContact(pageData, paramList, AppConstants.CONTACT_TYPE_CELL_CODE, AppConstants.CONTACT_TYPE_CELL, AppConstants.NCP_PARTICIPANT_TYPE);
			helper.populateContact(pageData, paramList, null, AppConstants.CONTACT_TYPE_EMAIL, AppConstants.NCP_PARTICIPANT_TYPE);
			...
			// collect Participant (NCP type) extra details to save in
			// ACTW_NCP_EXT
			// table.
			...
			if (request.getParameter("vehicleInfo") != null && request.getParameter("vehicleInfo").equals("1"))
			{
				...
				if (!vehicleLst.trim().equals(""))
				{
					...
					for (int i = 0; i < typeList.length; i++)
					{
						...
						if (request.getParameter("make_" + incType) != null && !request.getParameter("make_" + incType).equals(""))
						{
							vehicle.setMake(request.getParameter("make_" + incType));
						}
						if (request.getParameter("model_" + incType) != null && !request.getParameter("model_" + incType).equals(""))
						{
							vehicle.setModel(request.getParameter("model_" + incType));
						}
						if (request.getParameter("year_" + incType) != null && !request.getParameter("year_" + incType).equals(""))
						{
			...
			if (request.getParameter("isSupportOrder") != null && request.getParameter("isSupportOrder").equals("1"))
			{
				if (request.getParameter("support_order") != null)
				{
...

					{
						...
						{
						case 1:
							if (request.getParameter("court_dnumber") != null && !request.getParameter("court_dnumber").equals(""))
							{
								orderBean.setDocketNm(request.getParameter("court_dnumber"));

							}

							if (request.getParameter("court_odate") != null && !request.getParameter("court_odate").equals(""))
							{
								orderBean.setOrderEffDt(df.parse(request.getParameter("court_odate")));
							}

							if (request.getParameter("court_ostate") != null && !request.getParameter("court_ostate").equals(""))
							{
								orderBean.setState((request.getParameter("court_ostate")));

							}

							if (request.getParameter("court_ocnty") != null && !request.getParameter("court_ocnty").equals(""))
							{
								orderBean.setCounty((request.getParameter("court_ocnty")));

							}

							if (request.getParameter("court_oamt") != null && !request.getParameter("court_oamt").equals(""))
							{
								orderBean.setOrderAmount(Double.parseDouble(request.getParameter("court_oamt")));

							}

							if (request.getParameter("court_ofcy") != null && !request.getParameter("court_ofcy").equals(""))
							{
								orderBean.setPayFreq(request.getParameter("court_ofcy"));

							}

							if (request.getParameter("court_oamt_due") != null && !request.getParameter("court_oamt_due").equals(""))
							{
								orderBean.setAmtPastDue(Double.parseDouble(request.getParameter("court_oamt_due")));

							}
							if (request.getParameter("court_payor") != null && !request.getParameter("court_payor").equals(""))
							{
								orderBean.setPayorNm(request.getParameter("court_payor"));

							}

							if (request.getParameter("court_recip") != null && !request.getParameter("court_recip").equals(""))
							{
							...
							// mandatory, otherwise no need to save the court
							// order.
						...
						case 2:
							if (request.getParameter("medical_dnumber") != null && !request.getParameter("medical_dnumber").equals(""))
							{
								orderBean.setDocketNm(request.getParameter("medical_dnumber"));

							}

							if (request.getParameter("medical_odate") != null && !request.getParameter("medical_odate").equals(""))
							{
								orderBean.setOrderEffDt(df.parse(request.getParameter("medical_odate")));
							}

							if (request.getParameter("medical_ostate") != null && !request.getParameter("medical_ostate").equals(""))
							{
								orderBean.setState(request.getParameter("medical_ostate"));

							}

							if (request.getParameter("medical_ocnty") != null && !request.getParameter("medical_ocnty").equals(""))
							{
								orderBean.setCounty(request.getParameter("medical_ocnty"));

							}

							if (request.getParameter("medical_oamt") != null && !request.getParameter("medical_oamt").equals(""))
							{
								orderBean.setOrderAmount(Double.parseDouble(request.getParameter("medical_oamt")));

							}

							if (request.getParameter("medical_ofcy") != null && !request.getParameter("medical_ofcy").equals(""))
							{
								orderBean.setPayFreq(request.getParameter("medical_ofcy"));

							}

							if (request.getParameter("medical_oamt_due") != null && !request.getParameter("medical_oamt_due").equals(""))
							{
								orderBean.setAmtPastDue(Double.parseDouble(request.getParameter("medical_oamt_due")));

							}
							if (request.getParameter("medical_payor") != null && !request.getParameter("medical_payor").equals(""))
							{
								orderBean.setPayorNm(request.getParameter("medical_payor"));

							}

							if (request.getParameter("medical_recip") != null && !request.getParameter("medical_recip").equals(""))
							{
							...
							break;
						case 3:
							if (request.getParameter("spousal_dnumber") != null && !request.getParameter("spousal_dnumber").equals(""))
							{
								orderBean.setDocketNm(request.getParameter("spousal_dnumber"));

							}

							if (request.getParameter("spousal_odate") != null && !request.getParameter("spousal_odate").equals(""))
							{
								orderBean.setOrderEffDt(df.parse(request.getParameter("spousal_odate")));
							}

							if (request.getParameter("spousal_ostate") != null && !request.getParameter("spousal_ostate").equals(""))
							{
								orderBean.setState(request.getParameter("spousal_ostate"));

							}

							if (request.getParameter("spousal_ocnty") != null && !request.getParameter("spousal_ocnty").equals(""))
							{
								orderBean.setCounty(request.getParameter("spousal_ocnty"));

							}

							if (request.getParameter("spousal_oamt") != null && !request.getParameter("spousal_oamt").equals(""))
							{
								orderBean.setOrderAmount(Double.parseDouble(request.getParameter("spousal_oamt")));

							}

							if (request.getParameter("spousal_ofcy") != null && !request.getParameter("spousal_ofcy").equals(""))
							{
								orderBean.setPayFreq(request.getParameter("spousal_ofcy"));

							}

							if (request.getParameter("spousal_oamt_due") != null && !request.getParameter("spousal_oamt_due").equals(""))
							{
								orderBean.setAmtPastDue(Double.parseDouble(request.getParameter("spousal_oamt_due")));

							}
							if (request.getParameter("spousal_payor") != null && !request.getParameter("spousal_payor").equals(""))
							{
								orderBean.setPayorNm(request.getParameter("spousal_payor"));

							}

							if (request.getParameter("spousal_recip") != null && !request.getParameter("spousal_recip").equals(""))
							{
						...
						case 4:
							if (request.getParameter("voluntary_dnumber") != null && !request.getParameter("voluntary_dnumber").equals(""))
							{
								orderBean.setDocketNm(request.getParameter("voluntary_dnumber"));

							}

							if (request.getParameter("voluntary_odate") != null && !request.getParameter("voluntary_odate").equals(""))
							{
								orderBean.setOrderEffDt(df.parse(request.getParameter("voluntary_odate")));
							}

							if (request.getParameter("voluntary_ostate") != null && !request.getParameter("voluntary_ostate").equals(""))
							{
								orderBean.setState(request.getParameter("voluntary_ostate"));

							}

							if (request.getParameter("voluntary_ocnty") != null && !request.getParameter("voluntary_ocnty").equals(""))
							{
								orderBean.setCounty(request.getParameter("voluntary_ocnty"));

							}

							if (request.getParameter("voluntary_oamt") != null && !request.getParameter("voluntary_oamt").equals(""))
							{
								orderBean.setOrderAmount(Double.parseDouble(request.getParameter("voluntary_oamt")));

							}

							if (request.getParameter("voluntary_ofcy") != null && !request.getParameter("voluntary_ofcy").equals(""))
							{
								orderBean.setPayFreq(request.getParameter("voluntary_ofcy"));

							}

							if (request.getParameter("voluntary_oamt_due") != null && !request.getParameter("voluntary_oamt_due").equals(""))
							{
								orderBean.setAmtPastDue(Double.parseDouble(request.getParameter("voluntary_oamt_due")));

							}
							if (request.getParameter("voluntary_payor") != null && !request.getParameter("voluntary_payor").equals(""))
							{
								orderBean.setPayorNm(request.getParameter("voluntary_payor"));

							}

							if (request.getParameter("voluntary_recip") != null && !request.getParameter("voluntary_recip").equals(""))
							{
			...
			if (mode.equals(AppConstants.DB_CREATE))
			{
				...
				String ncpPartId = casePartService.CreateCaseParticipant(partInfo, ((ParamData) pageData), userId, AppConstants.NCP_PARTICIPANT_TYPE);

				if (ncpPartId != "")
				{
					String rValue = AppConstants.OPERATION_SUCCESS;
					// update step progress
					if (workFlowService.getStepNumber(partInfo.getApplicationId(), AppConstants.FLOW_TYPE_APPL_SCREEN) < 5)
					{
			...
			else if (mode.equals(AppConstants.DB_UPDATE))
			{
				...
				String rValue = casePartService.updateParticipant(partInfo, ((ParamData) pageData), userId, AppConstants.NCP_PARTICIPANT_TYPE);
