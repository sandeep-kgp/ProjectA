...
public class EcoaDaoAuthenticationProvider extends AbstractUserDetailsAuthenticationProvider
{
			...
			throws AuthenticationException
	{
		...
		if (loadedUser == null) { throw new AuthenticationServiceException("Invalid user credentials supplied. Please try again."); }
	 ...
	 * @throws AuthenticationException
	 */
	private UserDetails retrieveLoadedUser(String username, UsernamePasswordAuthenticationToken authentication,
			String userType) throws AuthenticationException
	{
		...
		try
		{
			...
			if (userType.equalsIgnoreCase("ncid") || userType.equalsIgnoreCase("pncid"))
			{
				if (!WebsiteConfiguration.getDeployedEnv().equalsIgnoreCase("PROD") && WebsiteConfiguration.getByPassNCIDAuth().equalsIgnoreCase("Y"))
				{
					...
					logger.info("\n********** WebsiteConfiguration.getByPassNCIDAuthentication() = " + WebsiteConfiguration.getDeployedEnv() + "*********\n");
