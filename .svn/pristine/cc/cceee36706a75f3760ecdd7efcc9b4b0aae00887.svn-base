...
public class CaseApplicationCPController extends BasicAnnotatedFormController
{
	...
	public String getCustodialInfo(HttpServletRequest request, Model model)
	{
		...
		CaseParticipant partApplicant = null;
		...
		try
		{
			...
			if (caseApp.getApplicantId() != null && !caseApp.getApplicantId().equals(""))
			{
				...
				if (cpPartInfo != null && cpPartInfo.size() > 0)
				{
					...
					// Added for AloaId :7261 which needs to show
					// income/employment section only applicant is CP Parent or
					// NCP.
					partApplicant = cpPartInfo.get(0);
				...
				if (cpPartInfo != null && cpPartInfo.size() > 0)
				{
					...
					if (partInfo != null && (partInfo.getEmpStatus().equalsIgnoreCase(AppConstants.SELOPTION_YES) || partInfo.getEmpStatus().equalsIgnoreCase(AppConstants.SELOPTION_NO)))
					{
			...
			model.addAttribute("partApplicant", partApplicant);
			...
			@ModelAttribute("partInfo") CaseParticipant partInfo)
	{
		...
		try
		{
			...
			// added unknown/blank to the employment section drop down .Aloa id:
			// 7261

			if (paramList.get("empStatus").equals(AppConstants.SELOPTION_YES))
			{
				helper.populateCurrEmpInfo(pageData, paramList, AppConstants.CP_PARTICIPANT_TYPE);
			}
			else
			{
				if (paramList.get("empStatus").equals(AppConstants.SELOPTION_NO))
