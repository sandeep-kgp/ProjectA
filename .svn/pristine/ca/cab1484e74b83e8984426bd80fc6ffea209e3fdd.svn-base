...
<script>
	...
	function validateform() {
		...
		if (document.OnlineAppCPForm.applicantFNm.value == "") {
			...
			return false;
		} else if (document.OnlineAppCPForm.specialAssistOt.value !="" && !alias_name_regex.test(document.OnlineAppCPForm.specialAssistOt.value.trim())){
			alert("Special characters (except comma) and numbers are not allowed.");
			document.OnlineAppCPForm.specialAssistOt.focus();
			return false;
		} 
			...
			return false;
		} else if (!document.getElementById("res_addr").checked  && document.OnlineAppCPForm.res_city.value !="" && !name_regex.test(document.OnlineAppCPForm.res_city.value.trim())){
			alert("Special characters and numbers are not allowed for city.");
			document.OnlineAppCPForm.res_city.focus();
			return false;
		}
		else if (document.getElementById('empStatus').value.trim() == "1" && document.OnlineAppCPForm.emp_city.value !="" && !name_regex.test(document.OnlineAppCPForm.emp_city.value.trim())){
			alert("Special characters and numbers are not allowed for city.");
			document.OnlineAppCPForm.emp_city.focus();
			return false;
		}
			...
			return false; 
		} else if (!document.getElementById("res_addr").checked && document.OnlineAppCPForm.res_zip.value.trim() != "" && !zip_regex.test(document.OnlineAppCPForm.res_zip.value)) {
			alert("Zip code should be all digits and 5 digits long.");
			document.OnlineAppCPForm.res_zip.focus();
			return false; 
		} else if (document.getElementById('empStatus').value.trim() == "1" && document.OnlineAppCPForm.emp_zip.value.trim() != "" && !zip_regex.test(document.OnlineAppCPForm.emp_zip.value)) {
			alert("Zip code should be all digits and 5 digits long.");
			...
			return false;			
		} else if (document.getElementById('empStatus').value.trim() == "1" && !validatePhone(document.getElementById('emp_ph1').value, 
			document.getElementById('emp_ph2').value, document.getElementById('emp_ph3').value)) {
			document.getElementById('emp_ph1').focus();
			return false;
		} else if (document.getElementById('empStatus').value.trim() === "2" && document.getElementById('dt_emp_end').value.trim() !== "" 
...
<form name="OnlineAppCPForm" id="OnlineAppCPForm" method="post" action="${pageContext.request.contextPath}/cssp/user/caseApplicationCPInfo.htm" modelAttribute="partInfo">
	...
	<div id="aplContent">
		<div class="gutter2">
				...
				<h3>SECTION 2- CUSTODIAL PARENT/GUARDIAN INFORMATION  <b><span  style="float:right;"><font size="2">Reference Number:${selectedApp}</font></span></b></h3>
				<hr />
							...
							<table>
								<tr>
									...
									<td><label>Social Security No: </label>
										<input type="text" title="Enter SSN in format XXXXXXXXX." name="ssnNb" value="${partInfo != null ? partInfo.getSsnNb() : ''}" size="9"
										maxlength="9">
									</td>
								<tr>
							</table>
			...
			<div>
				<fieldset>
					...
					<p>
						<input type="checkbox" name="res_addr" id= "res_addr" value="res_addr" ${resAddr == null  ? 'checked' : ''} onclick="checkResidAddr(this)"> Check if residential address is same as mailing address
					</p>
			...
			<br>
			<!-- Income Information -->
			<div id="incomeInfo" style="${partApplicant.getPartPrntGrdian().equals('1') || partApplicant.getPartType().equals('2')  ? 'display: block' : 'display: none'}" class=center>
				<div>
					<p align="left">
						<b><u>Income Information:</u></b>
					</p>
					
					<!-- aloa id: 7261 :update to add unknown/blank to employment drop down. -->
					<p align="left">
						<b>Is the custodial parent currently employed? </b> 
						<select id="empStatus" name="empStatus" title="Select from picklist. Provide additional information if applicable." onchange="toggleEmp(this)">
					           <c:forEach var="emplStatus" items="${seloptionLookup}">
					              <option value="${emplStatus.getCodeId()}" ${partInfo.getEmpStatus().equals(emplStatus.getCodeId()) ? 'selected' : ''}>${emplStatus.getCodeDesc()}</option>
					           </c:forEach>
		                 </select>
					</p>
				</div>
				<!-- current employer -->
				<div id="currEmp" style="display: ${partInfo.getEmpStatus().trim().equals('1') ? 'block' : 'none'}" class=center>
					<fieldset>
						<legend>
						...
						<table>
							<tr>
								<td colspan="5" align="left">
									<label>Name: </label>
									<input type="text" name="curr_emp" value="${empInfo != null && empInfo.getThirdPartyInfo().getThirdPartyType().equals('2')? empInfo.getThirdPartyInfo().getThirdPartyNm() : ''}" size="40" maxlength="40">
								</td>
							</tr>		
							<tr>
								<td colspan="5" align="left">
									<label>Street: </label>
									<input type="text" name="emp_street" id="emp_street" value="${empInfo != null ? empInfo.getThirdPartyInfo().getAddressLn1() : ''}" size="40" maxlength="40">
								</td>
							</tr>
							<tr>
								<td>	
				...
				<!-- last known employer -->
				<div id="lastEmp" style="display: ${partInfo.getEmpStatus().trim().equals('2') ? 'block' : 'none'}" class=center>
					<fieldset>
						<legend>
						...
						<table>
							<tr>
								<td colspan="3" align="left">
									<label>Name: </label>
									<input type="text" name="last_emp" id="last_emp" value="${empInfo != null && empInfo.getThirdPartyInfo().getThirdPartyType().equals('3')? empInfo.getThirdPartyInfo().getThirdPartyNm() : ''}" size="40" maxlength="40">
								</td>
							</tr>
							<tr>
								<td>
