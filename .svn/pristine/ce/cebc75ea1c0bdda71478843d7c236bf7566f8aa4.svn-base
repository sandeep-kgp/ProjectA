...
<script>
	...
	{	
		...
		if (document.getElementById('agency').value.trim() == "1" && document.OnlineAppApplicantInfo.agCity.value != "" && !name_regex.test(document.OnlineAppApplicantInfo.agCity.value.trim())){
			alert("Special characters and numbers are not allowed for city.");
			document.OnlineAppApplicantInfo.agCity.focus();
			return false;
		}
		
		if (document.getElementById('agency').value.trim() == "1" && document.OnlineAppApplicantInfo.agZip.value != "" && ! zip_regex.test(document.OnlineAppApplicantInfo.agZip.value.trim())){
			alert("Zip code should be all digits and 5 digits long.");
			document.OnlineAppApplicantInfo.agZip.focus();
			return false;
		}
...
<form name="OnlineAppApplicantInfo" id="OnlineAppApplicantInfo"	method="post" action="${pageContext.request.contextPath}/cssp/user/applicantInfo.htm" 
	...
	<div id="aplContent">
		...
		<div class="gutter2">
			...
			<div>
				<h3>SECTION 1- APPLICANT/PUBLIC ASSISTANCE RECIPIENT INFORMATION <b><span  style="float:right;"><font size="2">Reference Number:${selectedApp}</font></span></b></h3>
				<hr />
			...
			<fieldset>
				<p>
					...
					<select	id="enforecment" name ="enforcementStatus" onchange="toggleInfo(this)" title="Select from picklist. Enter additional information if applicable.">
						<c:forEach var="enforceStatus" items="${seloptionLookup}">
									<option value="${enforceStatus.getCodeId()}"
										${appInfo.getEnforceStatus().equals(enforceStatus.getCodeId()) ? 'selected' : ''}>${enforceStatus.getCodeDesc()}</option>
						</c:forEach>
					</select>
				</p>

				<div id="enforecment_info"
					style="display: ${appInfo.getEnforceStatus().equals('1') ? 'block' : 'none'}"
					class=center>
					<table>
						<tr>
			...
			<fieldset>
				<p title="Enter your private attorney details.">	Do you currently employ an attorney or private collection agency for child support? 
					<select name="attorneyStatus" id="agency"	onchange="toggleInfo(this)">
						<c:forEach var="attorStatus" items="${seloptionLookup}">
									<option value="${attorStatus.getCodeId()}"
										${appInfo.getAttorneyStatus().equals(attorStatus.getCodeId()) ? 'selected' : ''}>${attorStatus.getCodeDesc()}</option>
						</c:forEach>
					</select>
				</p>
