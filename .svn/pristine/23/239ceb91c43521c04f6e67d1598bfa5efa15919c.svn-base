...
public class CaseApplicationDaoImpl implements CaseApplicationDao
{
	...
	public void setDataSource(DataSource dataSource)
	{
		...
		// works
		procCaseApplication5 = new SimpleJdbcCall(jdbcTemplate).withoutProcedureColumnMetaDataAccess().withSchemaName(WebsiteConfiguration.getDbSchema()).withProcedureName("FKWEB_R_APPLID_CSS_APPL_REP");
	...
	private String buildDataParam(CaseApplication applBean)
	{
		...
		dataParam.append(applBean.getEnforceStatus().trim().replaceAll(fldSeperator, "") + fldSeperator);

		dataParam.append(applBean.getAttorneyStatus().trim().replaceAll(fldSeperator, "") + fldSeperator);
	...
	public List<CaseApplication> getCaseApplicationIdForReport(String applId, String ncId) throws SQLException
	{
		...
		procCaseApplication5.declareParameters(new SqlParameter("ID_APPLICATION", Types.CHAR), new SqlParameter("NCID", Types.CHAR)).returningResultSet("caseApp", new CaseApplicationRowMapper());

		SqlParameterSource in = new MapSqlParameterSource().addValue("ID_APPLICATION", applId).addValue("NCID", ncId);

		Map<String, Object> results = procCaseApplication5.execute(in);
