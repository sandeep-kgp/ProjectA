...
public class EcoaUserDetailsServiceImpl implements UserDetailsService
{
	...
	public UserDetails loadUserByUsername(String userName) throws UsernameNotFoundException
	{
		...
		//Aloa id:7104 for item #1: In case of forcing NCID by-pass authentication, should be used only in test environment
	...
	{
		logger.debug("\n**********IN EcoaUserDetailsServiceImpl: loadAndAuthenticate(" + userName + ",password:*****" + ",ipaddress" + ipaddress + "," + browserUserAgent + ")**********\n");
	...
	public UserDetails loadAndAuthenticate(String userName, String password, String ipaddress, String browserUserAgent,
			String userType)
	{
		logger.debug("\n**********IN EcoaUserDetailsServiceImpl: loadAndAuthenticate(" + userName + ",password:*****" + ",ipaddress" + ipaddress + "," + browserUserAgent + "authenticate: true , UserType" + userType + ")**********\n");
	...
	{
		logger.debug("\n**********IN EcoaUserDetailsServiceImpl: loadUser(" + username + ",password:*****" + ",ipaddress" + ipaddress + ",browserUserAgent" + browserUserAgent + "authenticate" + authenticate + ")**********\n");
		...
		try
		{
			if (authenticate)
			{
				...
				logger.info("***NCID authentication code = " + authenticationCode + ", authentication description = " + authDesc);

				if (authenticationCode.equals(AppConstants.NCID_AUTH_SUCCESS))
				{
					EcoaUser authNcIdUser = userService.getNCIdResponseUser(username);

					logger.info("************successfully received authenticated NCID user details for NCID user :" + username);

					String ncId = authNcIdUser.getLoginId();

					userDetails = new EcoaUserDetails(ncId, password, new ArrayList<GrantedAuthority>(), !isAccountExpired, !isAccountLocked, !isPasswordExpired, !isAccountDisabled, ipaddress, browserUserAgent, null, authNcIdUser, authDesc);
				}
				else
				{
					...
					{
						userDetails = new EcoaUserDetails(username, password, new ArrayList<GrantedAuthority>(), !isAccountExpired, !isAccountLocked, !isPasswordExpired, !isAccountDisabled, ipaddress, browserUserAgent, null, null, authDesc);
					}
			...
			{
				// Aloa id:7104 case of forcing NCID by-pass authentication, should be used only in test environment

				logger.info("\n********** By passing NCID authentication, this should be test environment only**********\n");
				userDetails = new EcoaUserDetails(username, password, new ArrayList<GrantedAuthority>(), !isAccountExpired, !isAccountLocked, !isPasswordExpired, !isAccountDisabled, ipaddress, browserUserAgent, null, null, "");
			}
		...
		catch (Exception ex)
		{
			...
			throw new AuthenticationServiceException("We are having technical problems. Please try login after few minutes. The Technical Team has been notified");
		}
			...
			boolean authenticate, String userType) throws AuthenticationException, DataAccessException
	{
		...
		try
		{
			if (authenticate)
			{
				...
				{
					logger.info("\n**********Authenticating NCID user : + userName  **********\n");
					...
					logger.info("***NCID authentication code = " + authenticationCode + ", authentication description = " + authDesc + " for NCID user:" + userName);

					if (authenticationCode.equals(AppConstants.NCID_AUTH_SUCCESS))
					{
						logger.info("***NCID authentication is successful for NCID user:" + userName);

						//fix for Aloa id:7104 item 3.

						logger.info("************before retrieving authenticated NCID user details for NCID user :" + userName);
					...
					else
					{
						...
						// appropriate
						// messages on the front end why user failed login.
						logger.info("***NCID authentication is failed for NCID user:" + userName);
				...
				if (userType.equalsIgnoreCase("tcaId"))
				{
					...
					if (!userName.matches("^[a-zA-Z0-9]+$")) { throw new DataFormatException("old useId  contains only characters and numbers."); }

					if (userName.length() < 5 || userName.length() > 10) { throw new DataFormatException("old userId should be between 5 and 10 characters"); }

					if (password.length() < 5 || password.length() > 10) { throw new DataFormatException("password should be between 5 and 10 characters"); }
					...
					if (preNcIdUser != null)
					{
						...
						userDetails = new EcoaUserDetails(preNcIdUser.getId_user(), password, new ArrayList<GrantedAuthority>(), !isAccountExpired, !isAccountLocked, !isPasswordExpired, !isAccountDisabled, ipaddress, browserUserAgent, preNcIdUser, null, "");
					}
		...
		{
			logger.info(unfex.getMessage());
			throw unfex;
		}
		catch (AuthenticationServiceException asex)
		{
			logger.info(asex.getMessage());
			throw asex;
		}
		catch (Exception ex)
		{
			...
			ErrorDescriptor errorBean = new ErrorDescriptor(this.getClass().getName(), Thread.currentThread().getStackTrace()[1].getMethodName(), ex.getMessage(), ex);

			String body = errorBean.getEmailBody();

			emailManager.sendEmail(WebsiteConfiguration.getEmailNotify(), AppConstants.MAIL_FROM, AppConstants.MAIL_ERROR_SUBJECT, body);

			throw new AuthenticationServiceException("A technical problem with login has been detected.  Please try to login later. An Administrator has been notified of the error.");
