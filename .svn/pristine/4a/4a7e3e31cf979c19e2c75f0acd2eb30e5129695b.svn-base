...
public class ApplicantInfoController extends BasicAnnotatedFormController
{
		...
		try
		{
			if (session.getAttribute(AppConstants.SELECTED_APPLICATION) != null)
			{
				...
				if (appInfo.getApplicantId() != null && !appInfo.getApplicantId().equals(""))
				{
					...
					if (appInfo.getAttorneyStatus().equals("1"))
	...
	public String processApplicant(HttpServletRequest request, HttpServletResponse response, Model model,
			@ModelAttribute("partInfo") CaseParticipant partInfo)
	{
		...
		try
		{
			...
			if (applBean.getApplicationId() != null && !applBean.getApplicationId().equals(""))
			{
				...
				//fix for aloa Id: 7022.
				if (request.getParameter("enforcementStatus") != null && !request.getParameter("enforcementStatus").equals(""))
				{
					applBean.setEnforceStatus(request.getParameter("enforcementStatus"));
				...
				if (request.getParameter("enforcementStatus") != null && request.getParameter("enforcementStatus").equals("1"))
				...
				if (request.getParameter("attorneyStatus") != null && !request.getParameter("attorneyStatus").equals(""))
				{
					applBean.setAttorneyStatus(request.getParameter("attorneyStatus"));

				}

				if (request.getParameter("attorneyStatus") != null && request.getParameter("attorneyStatus").equals("1"))
				{
				...
				if (mode.equals(AppConstants.DB_CREATE))
				{
					...
					if (rValue != null && rValue.substring(0, 4).trim().equals(AppConstants.OPERATION_SUCCESS))
					{
						...
						if (rValue != null && !rValue.substring(0, 4).trim().equals(AppConstants.OPERATION_SUCCESS)) { throw new Exception("ApplicantInfoController: processApplicant: create: update step number failed. Return code: " + rValue); }
					}
					else
					{
					...
					if (rValue != null && !rValue.substring(0, 4).trim().equals(AppConstants.OPERATION_SUCCESS)) { throw new Exception("ApplicantInfoController: processApplicant: create: insertOrUpdateSignature failed. Return code: " + rValue); }
				...
				if (rValue != null && !rValue.substring(0, 4).trim().equals(AppConstants.OPERATION_SUCCESS)) { throw new Exception("ApplicantInfoController: processApplicant: update: insertOrUpdateSignature failed. Return code: " + rValue); }
			}
