	...
	<div class="gutter">
	   ...
	   <c:choose>
            <c:when test="${fn:containsIgnoreCase(deployedEnv, 'DEV')}">
		      <ul><li>Register your account in NCID. Register here <a target="_blank" href=https://idpdev.nc.gov:8443/nidp/idff/sso?/> DEV NCID </a></li></ul>
	        </c:when>
	        <c:when test="${fn:containsIgnoreCase(deployedEnv, 'PREPROD')}">
		      <ul><li>Register your account in NCID. Register here <a target="_blank" href=https://idppre.nc.gov:8443/nidp/idff/sso?/> PRE-PROD NCID </a></li></ul>
	        </c:when>
	        <c:otherwise>
			  <ul>
		...
		<B><u>Your account already registered?</u></B>
			<ul><li><strong><a href="${pageContext.request.contextPath}/loginnp.htm?u=pNcId">Login with NCID</a></strong></li></ul>
		<B><u>Forgot your username/password?</u></B>
		<c:choose>
	          <c:when test="${fn:containsIgnoreCase(deployedEnv, 'DEV')}">
			   		<ul><li>Visit <a target="_blank" href=https://idpdev.nc.gov:8443/nidp/idff/sso?/> DEV NCID </a> to reset your username/password.</li></ul>
		      </c:when>
		      <c:when test="${fn:containsIgnoreCase(deployedEnv, 'PREPROD')}">
			   		<ul><li>Visit <a target="_blank" href=https://idppre.nc.gov:8443/nidp/idff/sso?/> PRE-PROD NCID </a> to reset your username/password.</li></ul>
		      </c:when>
	          <c:otherwise>
			   		<ul><li>Visit <a target="_blank" href=https://ncid.nc.gov />NCID</a> to reset your username/password.</li></ul>
		      </c:otherwise>
	    </c:choose>
		<B><u>Having trouble registering for NCID? </u></B>
		<ul>
			<li>Your first and last name are required.</li>
			<li>When entering your email address and confirming your email address, the addresses must be the same.</li>			
