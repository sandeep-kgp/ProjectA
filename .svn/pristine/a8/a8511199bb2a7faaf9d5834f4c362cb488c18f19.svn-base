...
public class CaseApplicationRowMapper implements RowMapper<CaseApplication>
{
	...
	public CaseApplication mapRow(ResultSet rs, int rowNum) throws SQLException
	{
		...
		if (rs.getString("CD_HAVE_APPL_ENF") != null && !rs.getString("CD_HAVE_APPL_ENF").equals(""))
		{
			caseApplication.setEnforceStatus(rs.getString("CD_HAVE_APPL_ENF").trim());
		}

		if (rs.getString("CD_HAVE_AN_ATTY") != null && !rs.getString("CD_HAVE_AN_ATTY").equals(""))
		{
			caseApplication.setAttorneyStatus(rs.getString("CD_HAVE_AN_ATTY").trim());
		}
