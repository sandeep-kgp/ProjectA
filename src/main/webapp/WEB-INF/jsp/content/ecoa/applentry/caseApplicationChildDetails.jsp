...
<script>
	...
	function toggleMarital(selectObject)
	{
		...
		if (value =="5" || value =="4" || value =="0"){
			...
			document.getElementById('chldRelaCty').value = "";
			var countyList = document.OnlineAppChildForm.chldRelaCnty;
			countyList.options[countyList.selectedIndex].selected=false;
			var stateList = document.OnlineAppChildForm.chldRelaSt;
			stateList.options[stateList.selectedIndex].selected=false;
	...
	function validateform() {
		...
		var zip_regex = /^\d{5}$/;
		var name_regex = /^[a-zA-Z ]+$/;
		var alpha_comma_regex = /^[a-zA-Z, ]+$/;
		var paternity_regex = /^\d+\.\d{2,2}$/;
		...
		else if (document.OnlineAppChildForm.specialAssistOt.value.trim() != "" && !alpha_comma_regex.test(document.OnlineAppChildForm.specialAssistOt.value.trim())){
			alert("Special characters (except comma) and numbers are not allowed.");
			document.OnlineAppChildForm.specialAssistOt.focus();
			return false;		
		    }
		...
		if (document.OnlineAppChildForm.vfname.value !="" && !name_regex.test(document.OnlineAppChildForm.vfname.value.trim())){
			alert("Special characters and numbers are not allowed in veteran's first name.");
			document.OnlineAppChildForm.vfname.focus();
			return false;
		}
		
	   if (document.OnlineAppChildForm.vlname.value !="" && !name_regex.test(document.OnlineAppChildForm.vlname.value.trim())){
			alert("Special characters and numbers are not allowed in veteran's last name.");
			document.OnlineAppChildForm.vlname.focus();
			return false;
		}
		...
		if (document.OnlineAppChildForm.chldMoMridTofname.value !="" && !name_regex.test(document.OnlineAppChildForm.chldMoMridTofname.value.trim())){
			alert("Special characters and numbers are not allowed in first name.");
			document.OnlineAppChildForm.chldMoMridTofname.focus();
			return false;
		}
		
		if (document.OnlineAppChildForm.chldMoMridTomname.value !="" && !name_regex.test(document.OnlineAppChildForm.chldMoMridTomname.value.trim())){
			alert("Special characters and numbers are not allowed in middle name.");
			document.OnlineAppChildForm.chldMoMridTomname.focus();
			return false;
		}
		
		if (document.OnlineAppChildForm.chldMoMridTolname.value !="" && !name_regex.test(document.OnlineAppChildForm.chldMoMridTolname.value.trim())){
			alert("Special characters and numbers are not allowed in last name.");
			document.OnlineAppChildForm.chldMoMridTolname.focus();
			return false;
		}
		
		
		if (document.OnlineAppChildForm.chldConcCty.value !="" && !name_regex.test(document.OnlineAppChildForm.chldConcCty.value.trim())){
			alert("Special characters and numbers are not allowed in city.");
			document.OnlineAppChildForm.chldConcCty.focus();
			return false;
		} 
		
		if (document.OnlineAppChildForm.chldBthCty.value !="" && !name_regex.test(document.OnlineAppChildForm.chldBthCty.value.trim())){
			alert("Special characters and numbers are not allowed in city.");
			document.OnlineAppChildForm.chldBthCty.focus();
			return false;
		} 
		
		 var chldRelMar = document.getElementById('marital').value.trim();
		if ((chldRelMar == "1" || chldRelMar == "2" || chldRelMar == "3") && document.OnlineAppChildForm.chldRelaCty.value !="" && !name_regex.test(document.OnlineAppChildForm.chldRelaCty.value.trim())){
			alert("Special characters and numbers are not allowed in city.");
			document.OnlineAppChildForm.chldRelaCty.focus();
			return false;
		} 
		...
		else if(document.OnlineAppChildForm.brthDt.value.trim() != "" && document.getElementById('paternityTst').value.trim() == '1' && document.OnlineAppChildForm.paterntyTstDt.value.trim() != "" && !isDateFromBeforeDateTo(document.OnlineAppChildForm.brthDt.value.trim(), document.OnlineAppChildForm.paterntyTstDt.value.trim()))
		{
		...
		else if (document.getElementById('paternityTst').value.trim() == "1" &&  !IsNumeric(document.OnlineAppChildForm.paterntyRes.value.trim())){
			alert("Paternity percentage must range from 00.00 to 100.00.");
			document.OnlineAppChildForm.paterntyRes.focus();
			return false;
		}
	 
	   else if(document.getElementById('paternityTst').value.trim() == "1" && IsNumeric(document.OnlineAppChildForm.paterntyRes.value.trim()) && document.OnlineAppChildForm.paterntyRes.value.trim()>100) {
		    alert("Paternity percentage must range from 00.00 to 100.00."); 
		    document.OnlineAppChildForm.paterntyRes.focus();
			return false;
		}
		
	  else if(document.getElementById('paternityTst').value.trim() == "1" && document.OnlineAppChildForm.paterntyRes.value.trim()!= "" && !paternity_regex.test(document.OnlineAppChildForm.paterntyRes.value.trim())) {
	    alert("Paternity percentage must have two digits after decimal.Ex: x.xx or xx.xx or xxx.xx ."); 
	    document.OnlineAppChildForm.paterntyRes.focus();
		return false;
	  }
	
		
		//check whether atleast one NCP is selected or added to request support for child.
		
	          var ncpChldAffil= document.getElementsByName('ncpchldAffil');
		      var j=0;
		      var noSelectNcp = false;
		      
		      for(var i=0;i<ncpChldAffil.length;i++) 
		    	  
				 if (!ncpChldAffil[i].checked)
					 {
					   j++;
					 }
			  
			     if(j==ncpChldAffil.length)
			       {
			    	  noSelectNcp = true;
			          	
				   }
		var ncpTable = document.getElementById('ncpaffilTable');
		var ncpAdded = (ncpTable.rows.length)-1;
        if(ncpAdded == 0 && (noSelectNcp))
		{
		  alert("Select or enter the name(s) of the parent(s) for whom support is being requested for this child.");
		  document.OnlineAppChildForm.ncpF.focus();
		  return false;
...
<form name="OnlineAppChildForm" id="OnlineAppChildForm" method="post" action="${pageContext.request.contextPath}/cssp/user/caseApplicationChildDetails.htm"  modelAttribute="partInfo">
	...
	<div id="aplContent">
		<div class="gutter2">
				...
				<h3>SECTION 3- CHILD INFORMATION <b><span  style="float:right;"><font size="2">Reference Number:${selectedApp}</font></span></b></h3>
				<hr />					
				...
				<table>
					...
					<td colspan="5">
						...
						<td><label colspan="2">Social Security No: </label>
							<input title="Enter SSN in format XXXXXXXXX." type="text" name="ssnNb" value="${partInfo != null ? partInfo.getSsnNb() : ""}" size="9" maxlength="9"> 
						</td>
					</tr>
					</table>
						</td>
			...
			<fieldset>
				...
				<tr>
					<td>
						<input type="checkbox" id='' name="ncpchldAffil" ${chldNcpMap.containsKey(ncpList.getApplicantId()) ? 'checked': '' } value="${ncpList.getApplicantId()}">
					</td>
			...
			<fieldset>
				<p title="Select from picklist. Enter additional information if applicable."><b>Was the child's mother married to anyone when the child was conceived or born?</b>&nbsp;
					<select name="mariedBth" id="mariedBth" onchange="toggleSel(this)" title="Select from picklist. Enter additional information if applicable.">
			...
			<fieldset>
				<p align="left" title="Select from picklist. Enter additional information if applicable.">
					...
					<select name="wedlock" id="wedlock" onchange="toggleWedlock(this)" title="Select from picklist. Enter additional information if applicable.">
						<c:forEach var="selOption" items="${seloptionLookup}">
				...
				<div id="wedlock_info" style="display: ${partProfile.getChldWdlck().equals('2') ? 'block' : 'none'}" class=center>
					<p align="left" title="Select from picklist. Enter additional information if applicable.">
						<b>Was an Affidavit of Parentage completed?</b> <select name="affidavit" title="Select from picklist. Enter additional information if applicable." id="affidavit" onchange="toggleSel(this)">
				...
				<p><b>City, State, County and Country where child was conceived:</b></p>
					...
					<table>
						...
						<tr>
							...
							<td align="left">
							<select name="chldBthCnty" title="Enter county where this child was born.">
									<c:forEach var="cnty" items="${countyLookup}">
				...
				<div id="paternityTst_info" style="display: ${partProfile.getPatnTst().equals('1') ? 'block' : 'none'}" class=center>
					<table>
						<tr>
							...
							<td align="left"><input type="text" title = "Future date is not allowed for paternity test date." name="paterntyTstDt" id="paterntyTstDt" value="${partPaternty.getPatnTestDtStrPg()}" size="10" maxlength="10"></td>
						</tr>
						<tr>
							<td align="left"><label>Results: </label></td>
							<td align="left"><input type="text" name="paterntyRes" id="paterntyRes" title ="Enter paternity results from 00.00 to 100.00." value="${partPaternty.getPatnResult()}" size="10" maxlength="6"></td>
						</tr>
					</table>
				</div>
				<br>
				<p title="Select from picklist. Enter additional information if applicable. "><b>Has paternity been established?</b>&nbsp;
					<select id="paterntyEst" name="paterntyEst" onchange="toggleSel(this)" title="Select from picklist. Enter additional information if applicable.">
						<c:forEach var="selOption" items="${seloptionLookup}">
				...
				<div id="paterntyEst_info" style="display: ${partProfile.getPatnEst().equals('1') ? 'block' : 'none'}" class=center>
					<table>
						...
						<tr>
							<td align="left"><label>County: </label></td>
							<td align="left">
							<select name="paterntyCnty" title="Select from the picklist.">
									<c:forEach var="cnty" items="${countyLookup}">
						...
						<tr>
							<td align="left"><label>State: </label></td>
							<td align="left">
								<select name="paterntySt" id="paterntySt" title="Select from the picklist.">
									<c:forEach var="state" items="${stateLookup}">
			...
			<fieldset>
				<p align="left">
					...
					<select name="marital" id="marital" onchange="toggleMarital(this)" title="Select from picklist. Enter additional information if applicable.">
				...
				<div id="marital_info" style="display:${partProfile==null || partProfile.getMaritalStatus().equals('0') || partProfile.getMaritalStatus().equals('4') || partProfile.getMaritalStatus().equals('5')  ? 'none' : 'block'}" class=center>
					<table>
						<tr>
							...
							<td align="left">
								<input type="text" name="maDt" id="maDt" value="${partProfile.getMaritalStatusDtStrPg()}" title="Future date is not allowed for parent's relationship date." size="10" maxlength="10">
							</td>
						</tr>
						<tr>
							<td align="left"><label>City: </label></td>
							<td align="left"><input type="text" name="chldRelaCty" id="chldRelaCty" value="${partProfileExtMap.containsKey('3') ? partProfileExtMap.get('3').getProfCty() : ''}" size="25" maxlength="25"></td>
						</tr>
						<tr>
							<td align="left"><label>County: </label></td>
							<td align="left">
