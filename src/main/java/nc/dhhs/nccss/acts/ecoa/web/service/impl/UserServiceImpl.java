...
public class UserServiceImpl implements UserService
{
	...
	public boolean userExists(String userId) throws Exception
	{
		...
		if (user != null && user.getUserId() != 0) { return true; }
	...
	public String createOrUpdateUserInfo(EcoaUser user, String dbOperationType, String userEvent) throws Exception
	{
		...
		if (returnCode == null || !returnCode.substring(0, 4).trim().equals(AppConstants.OPERATION_SUCCESS))
			throw new RuntimeException("createOrUpdateUserInfo: Insert/update failed, Rollback this transaction! " + "returnCode:" + returnCode);
	...
	{
		logger.debug("\n********** IN UserServiceImpl: preNCIDUserAuthentication(userName: " + userName + "," + "Password ) **********\n");

		return ((userDAO.authenticatePreNcIdUser(userName, password)).isEmpty() ? null : userDAO.authenticatePreNcIdUser(userName, password).get(0));
	}
	...
	private EcoaUser buildUserFromNCIDResponse(String loginId) throws Exception
	{
		...
		// fix for aloa id :7104 item#5

		slogInIDResponse = authenticationService.searchUserByLoginID(loginId);

		if (slogInIDResponse == null) {

		throw new Exception("NCID webservice could not send user details."); }

		EcoaUser user = new EcoaUser();

		if (slogInIDResponse.getSearchUserByLoginIDResult().getEntryArray().getEntry().get(0).getGUID() != null)
		{
			user.setNcIdGuid(slogInIDResponse.getSearchUserByLoginIDResult().getEntryArray().getEntry().get(0).getGUID().trim());
		}

		if (slogInIDResponse.getSearchUserByLoginIDResult().getEntryArray().getEntry().get(0).getFirstName() != null)
		{
			user.setNcidFirstNM(slogInIDResponse.getSearchUserByLoginIDResult().getEntryArray().getEntry().get(0).getFirstName().trim());
		}
		if (slogInIDResponse.getSearchUserByLoginIDResult().getEntryArray().getEntry().get(0).getMiddleInitial() != null)
		{
			user.setNcidMiddleNM(slogInIDResponse.getSearchUserByLoginIDResult().getEntryArray().getEntry().get(0).getMiddleInitial().trim());
		}

		if (slogInIDResponse.getSearchUserByLoginIDResult().getEntryArray().getEntry().get(0).getLastName() != null)
		{
			user.setNcidLastNM(slogInIDResponse.getSearchUserByLoginIDResult().getEntryArray().getEntry().get(0).getLastName().trim());
		}

		if (slogInIDResponse.getSearchUserByLoginIDResult().getEntryArray().getEntry().get(0).getEMail() != null)
		{
			user.setNcIdEmail(slogInIDResponse.getSearchUserByLoginIDResult().getEntryArray().getEntry().get(0).getEMail().trim());
		}

		if (slogInIDResponse.getSearchUserByLoginIDResult().getEntryArray().getEntry().get(0).getUserID() != null)
		{
			user.setLoginId(slogInIDResponse.getSearchUserByLoginIDResult().getEntryArray().getEntry().get(0).getUserID().trim());
		}

		if (slogInIDResponse.getSearchUserByLoginIDResult().getEntryArray().getEntry().get(0).getUserType() != null)
		{
			user.setUserType(slogInIDResponse.getSearchUserByLoginIDResult().getEntryArray().getEntry().get(0).getUserType().trim());
		}
